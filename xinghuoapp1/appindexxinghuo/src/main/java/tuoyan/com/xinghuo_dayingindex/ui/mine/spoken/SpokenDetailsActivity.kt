...
//        }
        ...
        btn_buy.setOnClickListener {
            if ("已售罄".equals(btn_buy.text.toString())) {
...
//                    } else
                    if (detail.isBuy == "1") {
                        Toast.makeText(
                            this,
                            "请用电脑登录星火英语官网" + WEB_BASE_URL_PC + "，进入四六级口语考试模考系统进行试音与考试",
                            Toast.LENGTH_LONG
                        ).show()
...
//            bindData(lessonDetail)
    ...
    private fun setData(data: SpokenDetail) {
        ...
        tv_date_info.text = "考试时间：${data.examStartTime}"

        tv_buyer_count.text = "已购 " + data.buyCount.toString()
        if (data.isBuy == "1") {
            btn_buy.text = "已购买"
...
//        tv_price.text =
//            if (!data.assembleKey.isNullOrEmpty() && "1" == data.assembleState) "${data.assemblePrice}元 拼团"
