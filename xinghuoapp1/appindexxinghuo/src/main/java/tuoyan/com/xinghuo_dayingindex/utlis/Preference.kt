...
import tuoyan.com.xinghuo_dayingindex.bean.Section
 ...
 * 时间：  2017/4/26.
 */
class Preference<T>(val content: Context, val name: String, val default: T) : ReadWriteProperty<Any?, T> {
    ...
    private fun get(name: String, default: T): T = with(prefs) {
        val res: Any = when (default) {
            ...
            is UserInfo -> Gson().fromJson(getString(name, Gson().toJson(default)), UserInfo::class.java)
            is Section -> Gson().fromJson(getString(name, Gson().toJson(default)), Section::class.java)
            is Grade -> Gson().fromJson(getString(name, Gson().toJson(default)), Grade::class.java)
//            is ArrayList<*> -> Gson().fromJson(getString(name, Gson().toJson(default)), object : TypeToken<List<DownloadInfo>>() {}.type)
            is LoginResponse -> Gson().fromJson(getString(name, Gson().toJson(default)), LoginResponse::class.java)
