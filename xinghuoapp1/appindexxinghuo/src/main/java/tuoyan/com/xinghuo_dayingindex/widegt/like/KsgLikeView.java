...
public class KsgLikeView extends AnimationLayout {
     ...
     */
    @Override
    public void addFavor(double currentLike) {
        // 非空验证
        if (mLikeRes.isEmpty()) {
            throw new NullPointerException("Missing resource file！");
        }
        // 随机获取一个资源
        int favorRes = Math.abs(mLikeRes.get((int) (currentLike%4)));
        // 生成 配置参数
    ...
    private AnimatorSet generateEnterAnimation(View child) {
        ...
        enterAnimation.playTogether(
                ObjectAnimator.ofFloat(child, ALPHA, 1f,1f, 1f,1f,1f));
//                ObjectAnimator.ofFloat(child, SCALE_X, 1f, 2f),
//                ObjectAnimator.ofFloat(child, SCALE_Y, 1f, 2f));
        // 加一些动画差值器
    ...
    private ValueAnimator generateCurveAnimation(View child) {
        // 起点 坐标
        PointF pointStart = new PointF(mRandom.nextInt(mViewWidth), mViewHeight);
        // 终点 坐标
        PointF pointEnd = new PointF(mViewWidth, 0);
        // 属性动画
    ...
    private PointF getTogglePoint(int scale) {
        ...
        // 减去100 是为了控制 x轴活动范围
        pointf.x = mRandom.nextInt((int) (mViewWidth - mPicWidth));
        // 再Y轴上 为了确保第二个控制点 在第一个点之上,我把Y分成了上下两半
        pointf.y = (mViewHeight ) / scale;
        return pointf;
    }
}
