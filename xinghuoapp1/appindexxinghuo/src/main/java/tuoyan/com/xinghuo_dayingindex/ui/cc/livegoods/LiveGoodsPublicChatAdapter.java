 ...
 * 直播带货公聊适配器
 */
public class LiveGoodsPublicChatAdapter extends RecyclerView.Adapter<LiveGoodsPublicChatAdapter.ChatViewHolder> {
...
private int[] colors=new int[]{Color.parseColor("#74BAFF"),Color.parseColor("#FFA3D9"),
        Color.parseColor("#FFBE83"),Color.parseColor("#86FFC0")};
    ...
    public LiveGoodsPublicChatAdapter(Context context) {
    ...
    public ChatViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == selfType) {
            // 展示自己发出去的公聊
            itemView = mInflater.inflate(R.layout.live_goods_portrait_chat_single, parent, false);
            ...
            // 展示收到的广播消息
            itemView = mInflater.inflate(R.layout.live_goods_protrait_system_broadcast, parent, false);
            return new ChatViewHolder(itemView, viewType);
        }
    }
    ...
    public void onBindViewHolder(final ChatViewHolder holder, @SuppressLint("RecyclerView") final int position) {
        ...
        if (chatEntity.isBroadcast()) {
            // 展示广播信息
            holder.tv_message.setText(chatEntity.getMsg());
        } else {
            // 判断聊天内容的状态，是否显示
...
//            }
            ...
            try {
                name = new SpannableString((URLDecoder.decode(chatEntity.getUserName(), "UTF-8")));
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
            }
            name.setSpan(new ForegroundColorSpan(colors[position%4]), 0, name.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
//            holder.tv_message.setMovementMethod(LinkMovementMethod.getInstance());
            // 2. 如果聊天内容是正常的内容，就直接展示聊天内容
            String msg = chatEntity.getMsg();
            String url = null;
            int start = -1;
            ...
            SpannableString ss = new SpannableString(msg);
            //如果确定有url 需要匹配url之前的uri_ 并删除
            ss.setSpan(new ForegroundColorSpan(Color.parseColor("#ffffff")), 0, msg.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            ...
            if (url != null) {
                ...
                ss.setSpan(new ForegroundColorSpan(Color.parseColor("#ffffff")), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }

            SpannableStringBuilder ssb = new SpannableStringBuilder();
            ssb.append(name).append("  ").append(EmojiUtil.parseFaceMsg(mContext, ss));
            if (ssb!=null&&holder!=null&&holder.tv_message!=null) {

                holder.tv_message.setText(ssb);
            }
