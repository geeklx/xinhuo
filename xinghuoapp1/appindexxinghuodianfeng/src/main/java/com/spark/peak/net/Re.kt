...
import com.spark.peak.utlis.log.RequestFilterInterceptor
...
object Re {
    val client by lazy {
        OkHttpClient.Builder()
                ...
                .addNetworkInterceptor(RequestFilterInterceptor())//重复请求过滤器
    ...
    private fun networkInterceptor(): (Interceptor.Chain) -> Response = {
        //缓存拦截
        val request = it.request().newBuilder().removeHeader("User-Agent").addHeader("User-Agent", getUserAgent()).build()
        when {
            NetWorkUtils.isNetWorkReachable() -> it.proceed(request)
            request.method().toLowerCase() == "get" -> it.proceed(request.newBuilder().cacheControl(CacheControl.FORCE_CACHE).build())
    ...
    private fun tokenInterceptor(): (Interceptor.Chain) -> Response = {
        ...
        if (request.header("authorization") != null) {
            ...
            if (NetWorkUtils.isNetWorkReachable()) {//缓存策略
                response.newBuilder().removeHeader("Pragma").header("Cache-Control", "private, max-age=" + 0).build()
            } else {
                response.newBuilder().removeHeader("Pragma").header("Cache-Control", "private, only-if-cached, max-stale=" + (60 * 60 * 24 * 7 * 4)).build()
            }
            response
        }
    }
